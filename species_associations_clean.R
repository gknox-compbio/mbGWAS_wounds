...
p = ggplot(mtemp, aes(Genotype, Hill, group = Genotype)) + geom_boxplot(aes(fill = Genotype), outlier.shape = NA) + 
  facet_grid(~ SNP, scales = "free_x") + 
  labs(x = "Genotype per SNP", y = expression(Hill[1])) + geom_jitter(size = 1.5, alpha = 0.5, height = 0, width = 0.2) +
...
p = ggplot(res, aes(rs8031916, res, group = rs8031916)) + geom_boxplot(aes(fill = rs8031916), outlier.shape = NA) + 
  ...
  labs(x = "Genotype at rs8031916", y = "Residual Relative Abundance") + scale_fill_manual(values = cb)
...
d.arrows = data.frame(x = rep(0,length(keep)),y = rep(0,length(keep)), xend = c[keep,1]*.5, yend = c[keep,2]*.5, 
                      labels = rownames(c)[keep], p = postn)
d.arrows$labels = gsub("\\;.*", "", d.arrows$labels)
d.arrows$labels = gsub("\\.", " ", d.arrows$labels)

## ordination plot
p = ggplot(dat, aes(MDS1, MDS2)) + geom_point(aes(color = rs8031916), size = 2.5, alpha = 0.6) +
  scale_color_manual(values = cb)
p = p + labs(x = paste("MDS1 (", round(s[2,1]*100, digits =2), "%)", sep = ''),
             y = paste("MDS2 (", round(s[2,2]*100, digits =2), "%)", sep = ''))
p = p + annotate("segment", x = d.arrows$x, xend = 1.1*d.arrows$xend, y = d.arrows$y, 
                 yend = 1.1*d.arrows$yend, size = 1.15, alpha = 0.6, arrow = arrow(length=unit(0.2, "cm")))
p = p + geom_text_repel(data = d.arrows, aes(x = 1.1*xend, y = 1.1*yend, 
                                             label = str_wrap(labels)) , size = 3, point.padding = 0.5,
                        fontface = "italic") +
...
p1 = ggplot(dat, aes(rs8031916, MDS1, fill = rs8031916)) + geom_boxplot(aes(fill = rs8031916)) + scale_fill_manual(values = cb) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  coord_flip() + labs(x = "", y = "") + theme(legend.position = "none", axis.text = element_blank())
p2 = ggplot(dat, aes(rs8031916, MDS2, fill = rs8031916)) + geom_boxplot(aes(fill = rs8031916)) + scale_fill_manual(values = cb) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  labs(x = "", y = "") + theme(legend.position = "none", axis.text = element_blank())
...
print(grid.arrange(p, p1, p2, 
                   layout_matrix = rbind(c(NA ,1, 1, 1, 1, 1 ,1),
                                         c(3, 1, 1, 1, 1, 1 ,1), 
                                         c(3, 1, 1, 1, 1, 1 ,1), 
                                         c(3, 1, 1, 1, 1, 1, 1), 
                                         c(3, 1, 1, 1, 1, 1, 1), 
...
hill = data.frame(rich = c(one, four), 
                  group = c(rep("Se", length(one)), 
                            rep("Pa", length(four))))
hill$group = ordered(hill$group, levels = c("Pa", "Se"))

# p-value annotation from t-test below
p = ggplot(hill, aes(group, rich, group = group)) + geom_boxplot(aes(fill = group), outlier.size = NA) +
  geom_jitter(height = 0, width = 0.2, size = 2.5, alpha = 0.5) + 
  ylim(1,8) + labs(x = "Community", y = expression(Hill[1])) + theme_bw(base_size = 15) + 
  theme(axis.text = element_text(size = 10), axis.title = element_text(size = 11)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  theme(legend.position = "none") + scale_fill_manual(values = cb) + 
...
print(grid.arrange(arrangeGrob(p4, top = text_grob("a", x = unit(0.025, "npc"), y = unit(0.8, "npc"), just = c("left", "top"), size = 12)), 
                   ...
                   layout_matrix = rbind(c(1 ,1, 1, 1, NA, 2, 2 ,2, 2, 2),
...
for (i in unique(names(otu))){
  if(length(which(otu[,i] > 0)) > 0.05*dim(otu)[1]){ keep1 = "yes"} else {
    keep1 = "no"
  }
...
p = ggplot(data = net3, aes(from_id = from_id2,to_id = to_id2)) + theme_net() +
  geom_net(directed = FALSE, labelon = TRUE, repel = FALSE, ecolour = net3$col, labelcolour = "white", size = 10,
...
p = ggplot(dat.node, aes(Id, Count, fill = Cor)) + geom_bar(stat = "identity") + coord_flip() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  labs(y = "Correlation Counts", x = "") + theme(axis.text = element_text(size = 9), legend.text = element_text(size = 11)) + 
  cbbFill2 + guides(fill = guide_legend(title = "Correlation")) + theme(legend.title = element_text(size = 12), axis.title.x = element_text(size = 12))
p8 = p
...
p = ggplot(dat.nod, aes(Id, sum, fill = cor)) + geom_bar(stat = "identity") + coord_flip() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  ...
  cbbFill2 + guides(fill = guide_legend(title = "Correlation")) + 
